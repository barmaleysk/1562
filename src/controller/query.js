        ...
        Flower.find({category: query}).limit(limit).skip(limit * (page - 1)).then(result => {
          ...
          Promise.all(promises)
            .then(() => {
              ...
              return bot.sendMessage(user.userId, `Подробнее - получить детальную информацию по услуге.
Четыре простых шага:
1.Нажать ➕ для добавления услуги в раздел "заказы",
2.Нажать на кнопку 🛒 для перехода в раздел "заказы",
3.Подтвердить заказ
4.Ввести данные и получить Личного оператора, для дальнейшей работы 🤗
/ads
/packaging 
/content
